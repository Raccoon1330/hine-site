(function () {
    ...
    var INNER_HTML =
        ...
        +     '<div class="dm-row-item" id="dm-open-cloud-sync" style="cursor:pointer">'
        +       '<div class="dm-row-icon violet"><i class="fas fa-cloud-arrow-up"></i></div>'
        +       '<div class="dm-row-info"><div class="dm-row-title">跨设备云同步</div><div class="dm-row-desc">外观+纪念日+字卡+开屏次数</div></div>'
        +       '<i class="fas fa-chevron-right dm-tile-arrow"></i>'
        +     '</div>'
        +   '</div>'
    ...
    var DRAWER_CLOUD_HTML =
        '<div class="dm-action-drawer" id="dm-drawer-cloud">'
        +   '<div class="dm-drawer-backdrop" id="dm-drawer-cloud-backdrop"></div>'
        +   '<div class="dm-drawer-sheet" style="max-height:85vh;overflow-y:auto">'
        +     '<div class="dm-drawer-handle"></div>'
        +     '<div class="dm-drawer-title">'
        +       '<div class="dm-drawer-title-icon" style="background:linear-gradient(135deg,#9C6FD4,#7A4FB8);color:#fff"><i class="fas fa-cloud-arrow-up"></i></div>'
        +       '<div><div class="dm-drawer-title-text">外观云同步</div><div class="dm-drawer-subtitle">GitHub Gist 跨设备同步</div></div>'
        +     '</div>'
        +
        ...
        +         '<label style="display:block;font-size:12px;color:var(--text-secondary);margin-bottom:6px;font-weight:500">GitHub Token</label>'
        +         '<input type="password" id="cloud-sync-token" placeholder="ghp_xxxxxxxxxxxx" autocomplete="off" style="width:100%;padding:10px 14px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);color:var(--text-primary);font-size:13px;outline:none;box-sizing:border-box">'
        +       '</div>'
        +
        +       '<div style="margin-bottom:16px">'
        +         '<label style="display:block;font-size:12px;color:var(--text-secondary);margin-bottom:6px;font-weight:500">同步密码（两台设备需一致）</label>'
        +         '<input type="text" id="cloud-sync-password" placeholder="任意字符串，用于识别同步对" autocomplete="off" style="width:100%;padding:10px 14px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);color:var(--text-primary);font-size:13px;outline:none;box-sizing:border-box">'
        +       '</div>'
        +
        +       '<div id="cloud-sync-status" style="min-height:20px;font-size:12px;margin-bottom:12px"></div>'
        +
        +       '<div style="display:flex;gap:10px;margin-bottom:10px">'
        +         '<button class="dm-drawer-action-btn" id="cloud-sync-upload" style="flex:1">'
        +           '<div class="dm-drawer-btn-icon"><i class="fas fa-upload"></i></div>'
        +           '<div class="dm-drawer-btn-text"><div class="dm-drawer-btn-title">上传外观</div><div class="dm-drawer-btn-desc">当前设备 → 云端</div></div>'
        +         '</button>'
        +         '<button class="dm-drawer-action-btn primary" id="cloud-sync-download" style="flex:1;background:linear-gradient(135deg,#9C6FD4,#7A4FB8);border-color:#9C6FD4">'
        +           '<div class="dm-drawer-btn-icon"><i class="fas fa-download"></i></div>'
        +           '<div class="dm-drawer-btn-text"><div class="dm-drawer-btn-title">下载外观</div><div class="dm-drawer-btn-desc">云端 → 当前设备</div></div>'
        +         '</button>'
        +       '</div>'
        +
        +       '<button class="dm-drawer-cancel" id="dm-drawer-cloud-cancel" style="margin-top:8px">取消</button>'
        +     '</div>'
        +   '</div>'
        + '</div>';
    ...
    function ensureDrawersOnBody() {
        ...
        DRAWER_IDS.forEach(function(id) {
            ...
            else dummy.innerHTML = DRAWER_CLOUD_HTML;
    ...
    function openCloudSyncDrawer() {
        openDrawer('dm-drawer-cloud');

        setTimeout(function() {
            var drawer = document.getElementById('dm-drawer-cloud');
            if (!drawer) return;

            var tokenInput = drawer.querySelector('#cloud-sync-token');
            var pwdInput = drawer.querySelector('#cloud-sync-password');
            var statusEl = drawer.querySelector('#cloud-sync-status');

            if (!tokenInput || !pwdInput) return;

            tokenInput.value = localStorage.getItem('hine_cloud_token') || '';
            pwdInput.value = localStorage.getItem('hine_cloud_password') || '';

            var backdrop = drawer.querySelector('#dm-drawer-cloud-backdrop');
            var cancelBtn = drawer.querySelector('#dm-drawer-cloud-cancel');
            if (backdrop) backdrop.onclick = function() { closeDrawer('dm-drawer-cloud'); };
            if (cancelBtn) cancelBtn.onclick = function() { closeDrawer('dm-drawer-cloud'); };

            drawer.querySelector('#cloud-sync-upload').onclick = async function() {
                var token = tokenInput.value.trim();
                var pwd = pwdInput.value.trim();
                if (!token || !pwd) {
                    statusEl.innerHTML = '<span style="color:#ff5050">请填写 Token 和同步密码</span>';
                    return;
                }
                localStorage.setItem('hine_cloud_token', token);
                localStorage.setItem('hine_cloud_password', pwd);
                statusEl.innerHTML = '<span style="color:var(--text-secondary)"><i class="fas fa-spinner fa-spin"></i> 正在上传...</span>';
                try {
                    if (!window.CloudSync) throw new Error('同步模块未加载');
                    var result = await CloudSync.upload(token, pwd);
                    statusEl.innerHTML = '<span style="color:#3BC8A4"><i class="fas fa-check-circle"></i> 外观已' + (result.created ? '创建' : '更新') + '到云端 (Gist: ' + result.gistId.substring(0, 8) + '...)</span>';
                } catch(e) {
                    statusEl.innerHTML = '<span style="color:#ff5050"><i class="fas fa-times-circle"></i> ' + e.message + '</span>';
                }
            };

            drawer.querySelector('#cloud-sync-download').onclick = async function() {
                var token = tokenInput.value.trim();
                var pwd = pwdInput.value.trim();
                if (!token || !pwd) {
                    statusEl.innerHTML = '<span style="color:#ff5050">请填写 Token 和同步密码</span>';
                ...
                localStorage.setItem('hine_cloud_token', token);
                localStorage.setItem('hine_cloud_password', pwd);
                statusEl.innerHTML = '<span style="color:var(--text-secondary)"><i class="fas fa-spinner fa-spin"></i> 正在拉取并应用...</span>';
                try {
                    if (!window.CloudSync) throw new Error('同步模块未加载');
                    await CloudSync.download(token, pwd);
                    statusEl.innerHTML = '<span style="color:#3BC8A4"><i class="fas fa-check-circle"></i> 外观已同步，页面即将刷新...</span>';
                } catch(e) {
                    statusEl.innerHTML = '<span style="color:#ff5050"><i class="fas fa-times-circle"></i> ' + e.message + '</span>';
                }
            };
    ...
    function bindAll(mc) {
        ...
        var tileCloud = mc.querySelector('#dm-open-cloud-sync');
        if (tileCloud) tileCloud.addEventListener('click', function () { openCloudSyncDrawer(); });
